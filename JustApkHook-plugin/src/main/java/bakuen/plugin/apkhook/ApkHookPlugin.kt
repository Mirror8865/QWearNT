    ...
    override fun apply(project: Project) {
        ...
        project.tasks.create("HookApk") { task ->
            ...
            task.doLast {
                ...
                Config.debugOutputDir.listFiles()?.find { it.name.lowercase().endsWith(".apk") }?.let {
                    brut.apktool.Main.main(arrayOf("d", it.absolutePath, "-o", Config.srcApkDir.absolutePath, "-f"))
                ...
                brut.apktool.Main.main(arrayOf("b", Config.targetCopyDir.absolutePath, "-o", File(project.projectDir, "hook/hook.apk").absolutePath, "-f"))
...
fun <T> Iterable<T>.join(transform: ((T) -> CharSequence)? = null) = joinToString(separator = "", transform = transform)